...
export default function TextSetter() {
  const { object: obj, editor } = useContext(GlobalStateContext);
  ...
  return (
    <>
      <Form form={form} onValuesChange={handleValuesChange} colon={false}>
        <FormItem name="fontFamily" label={t("setter.text.font_family")}>
          <Select options={FONT_PRESET_FAMILY_LIST} />
        </FormItem>
        <FormItem name="fontSize" label={t("setter.text.font_size")}>
          <SliderInputNumber
            max={400}
            onChangeComplete={() => {
              editor.fireCustomModifiedEvent();
        ...
        <FormItem name="fill" label={t("setter.text.fill")}>
          <ColorSetter type="fontColor" defaultColor="#000000" />
        </FormItem>
        <FormItem name="textAlign" label={t("setter.text.text_align")}>
          <AlignSetter />
        </FormItem>
        <FormItem name="fontStyles" label={t("setter.text.font_styles")}>
          <FontStyleSetter />
        </FormItem>
        <FormItem name="charSpacing" label={t("setter.text.char_spacing")}>
          <SliderInputNumber
            min={-200}
            max={800}
            onChangeComplete={() => {
        ...
        <FormItem name="lineHeight" label={t("setter.text.line_height")}>
          <SliderInputNumber
            min={0.5}
            max={2.5}
            step={0.01}
